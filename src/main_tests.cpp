...
static void fix_merge_mining_tag(BlockTemplate &block) {
	if (block.major_version >= 2) {
		...
		mmTag.merkleRoot = get_auxiliary_block_header_hash(block);
		if (!cryptonote::appendMergeMiningTagToExtra(block.parent_block.base_transaction.extra, mmTag))
			throw std::runtime_error("cryptonote::appendMergeMiningTagToExtra failed");
	}
}
...
void test_blockchain(common::CommandLine & cmd){
	...
	platform::DB::delete_db(config.coin_directory + "/blockchain");
	...
	if( !currency.parse_account_address_string("21mQ7KPdmLbjfpg3Coayi4hZzAEgjeL87QXGeDTHahKeJsvKHc6DoprAJmqUcLhWTUXtxCL6rQFSwEUe6NZdEoqZNpSq1iC", address))
	...
	for(int i = 0; i != 100; ++i){
		...
		Height height = 0;
		if( !block_chain.create_mining_block_template(block, address, cryptonote::BinaryArray{}, difficulty, height) )
		...
		std::cout << "ts=" << block.timestamp << " mts=" << info.timestamp_median << " height=" << info.height << " bid=" << common::pod_to_hex(info.hash) << std::endl;
	}
	// Mine alternatives
	block_chain.test_print_structure();
	for(int i = 0; i != 100; ++i){

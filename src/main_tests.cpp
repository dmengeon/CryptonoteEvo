...
static void fix_merge_mining_tag(BlockTemplate &block) {
	if (block.major_version >= 2) {
		...
		if (!cryptonote::append_merge_mining_tag_to_extra(block.parent_block.base_transaction.extra, mmTag))
			throw std::runtime_error("cryptonote::append_merge_mining_tag_to_extra failed");
	}
}

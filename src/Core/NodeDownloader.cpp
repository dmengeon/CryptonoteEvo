...
void Node::DownloaderV11::on_connect(P2PClientCryptonote *who) {
...
void Node::DownloaderV11::on_disconnect(P2PClientCryptonote *who) {
	if (who->is_incoming())
		return;
...
void Node::DownloaderV11::on_msg_notify_request_chain(P2PClientCryptonote *who,
...
void Node::DownloaderV11::advance_chain() {
	if (m_chain_client || !m_chain.empty())
		return;
	std::vector<P2PClientCryptonote *> lagging_clients;
	std::vector<P2PClientCryptonote *> sorted_clients;
	...
	std::sort(sorted_clients.begin(), sorted_clients.end(), [](P2PClientCryptonote *a, P2PClientCryptonote *b) -> bool {
...
void Node::DownloaderV11::on_msg_notify_request_objects(P2PClientCryptonote *who,
	...
	for (auto &&rb : req.blocks) {
		...
		try {
			...
			bid = cryptonote::get_block_hash(bheader);
...
void Node::DownloaderV11::advance_download(Hash last_downloaded_block) {
	...
	std::map<P2PClientCryptonote *, size_t> who_downloaded_counter;
	...
	for (auto &&dc : m_download_chain) {
		...
		if (total_downloading_blocks >= TOTAL_DOWNLOAD_BLOCKS)
			break;
		P2PClientCryptonote *ready_client = nullptr;
...
void Node::DownloaderV1::on_connect(P2PClientCryptonote *who) {
...
void Node::DownloaderV1::on_disconnect(P2PClientCryptonote *who) {
	if (who->is_incoming())
		return;
	m_good_clients.erase(who);
...
void Node::DownloaderV1::advance_download(Hash last_downloaded_block) {
	...
	if (!m_sync_client) {  // start sync with some client who has better chain
		P2PClientCryptonote *worst_client = nullptr;
		for (auto &&who : m_good_clients) {
			if (!worst_client ||
...
void Node::DownloaderV1::on_msg_notify_request_chain(P2PClientCryptonote *who,
    const NOTIFY_RESPONSE_CHAIN_ENTRY::request &req) {
...
void Node::DownloaderV1::on_msg_notify_request_objects(P2PClientCryptonote *,
    const NOTIFY_RESPONSE_GET_OBJECTS::request &req) {
	if (!m_sync_sent)
		return;

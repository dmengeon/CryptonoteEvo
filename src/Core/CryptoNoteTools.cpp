...
Hash cryptonote::get_base_transaction_hash(const BaseTransaction &tx) {
	if (tx.version < 2)
...
void cryptonote::decompose_amount(Amount amount, Amount dust_threshold, std::vector<Amount> &decomposed_amounts) {
...
size_t cryptonote::get_maximum_tx_size(size_t input_count, size_t output_count, size_t mixin_count) {

...
//pos stuff	
const std::vector<Amount> Currency::POWERS_OF_TEN = {
	1,
	10,
	100,
	1000,
...
Hash cryptonote::get_transaction_inputs_hash(const TransactionPrefix &tx) {
	BinaryArray ba = seria::to_binary(tx.inputs);
	Hash new_hash  = crypto::cn_fast_hash(ba.data(), ba.size());
	return new_hash;
}

Hash cryptonote::get_transaction_prefix_hash(const TransactionPrefix &tx) {
...
Hash cryptonote::get_transaction_hash(const Transaction &tx) {

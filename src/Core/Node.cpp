...
Node::Node(logging::ILogger &log, const Config &config, BlockChainState &block_chain)
	...
	if (!config.cryptonoted_bind_ip.empty() && config.cryptonoted_bind_port != 0)
		m_api.reset(new http::Server(config.cryptonoted_bind_ip, config.cryptonoted_bind_port,
...
void Node::sync_transactions(P2PClientCryptonote *who) {
...
void Node::P2PClientCryptonote::on_msg_bytes(size_t, size_t) {  // downloaded. uploaded
	//    node->peers.on_peer_bytes(get_address(), downloaded, uploaded,
	//    node->p2p.get_local_time());
}

CORE_SYNC_DATA
Node::P2PClientCryptonote::get_sync_data() const {
...
std::vector<PeerlistEntry> Node::P2PClientCryptonote::get_peers_to_share() const {
	auto result =
	    m_node->m_peer_db.get_peerlist_to_p2p(m_node->m_p2p.get_local_time(), config.p2p_default_peers_in_handshake);
	return result;
}

void Node::P2PClientCryptonote::on_first_message_after_handshake() {
...
void Node::P2PClientCryptonote::on_msg_handshake(COMMAND_HANDSHAKE::request &&req) {
	NetworkAddress addr;
...
void Node::P2PClientCryptonote::on_msg_handshake(COMMAND_HANDSHAKE::response &&req) {
...
void Node::P2PClientCryptonote::on_msg_notify_request_chain(NOTIFY_REQUEST_CHAIN::request &&req) {
...
void Node::P2PClientCryptonote::on_msg_notify_request_chain(NOTIFY_RESPONSE_CHAIN_ENTRY::request &&req) {
	m_node->m_downloader.on_msg_notify_request_chain(this, req);
}

void Node::P2PClientCryptonote::on_msg_notify_request_objects(NOTIFY_REQUEST_GET_OBJECTS::request &&req) {
...
void Node::P2PClientCryptonote::on_msg_notify_request_objects(NOTIFY_RESPONSE_GET_OBJECTS::request &&req) {
	m_node->m_downloader.on_msg_notify_request_objects(this, req);
}

void Node::P2PClientCryptonote::on_disconnect(const std::string &ban_reason) {
...
void Node::P2PClientCryptonote::on_msg_notify_request_tx_pool(NOTIFY_REQUEST_TX_POOL::request &&req) {
...
void Node::P2PClientCryptonote::on_msg_timed_sync(COMMAND_TIMED_SYNC::request &&req) {
	m_node->m_downloader.on_msg_timed_sync(req.payload_data);
}
void Node::P2PClientCryptonote::on_msg_timed_sync(COMMAND_TIMED_SYNC::response &&req) {
	m_node->m_downloader.on_msg_timed_sync(req.payload_data);
}

void Node::P2PClientCryptonote::on_msg_notify_new_block(NOTIFY_NEW_BLOCK::request &&req) {
...
void Node::P2PClientCryptonote::on_msg_notify_new_transactions(NOTIFY_NEW_TRANSACTIONS::request &&req) {
...
void Node::P2PClientCryptonote::on_msg_network_state(COMMAND_REQUEST_NETWORK_STATE::request &&req) {
...
void Node::P2PClientCryptonote::on_msg_stat_info(COMMAND_REQUEST_STAT_INFO::request &&req) {
	if (!m_node->check_trust(req.tr)) {
		disconnect(std::string());
		return;
	}
...
void Node::advance_long_poll() {
	...
	if (m_long_poll_http_clients.empty())
		return;
	api::cryptonoted::GetStatus::Response resp = create_status_response3();
	...
	for (auto lit = m_long_poll_http_clients.begin(); lit != m_long_poll_http_clients.end();) {
		const bool method_status = lit->original_json_request.get_method() == api::cryptonoted::GetStatus::method() ||
		                           lit->original_json_request.get_method() == api::cryptonoted::GetStatus::method2();
		...
		if (method_status) {
			...
			try {
				api::cryptonoted::GetBlockTemplate::Request gbt_req;
				lit->original_json_request.load_params(gbt_req);
				api::cryptonoted::GetBlockTemplate::Response gbt_res;
...
</svg></td><td>cryptonoted &bull; version
)";
...
bool Node::on_api_http_request(http::Client *who, http::RequestData &&request, http::ResponseData &response) {
	...
	if (!m_config.cryptonoted_authorization.empty() &&
	    request.r.basic_authorization != m_config.cryptonoted_authorization) {
...
const std::unordered_map<std::string, Node::HTTPHandlerFunction> Node::m_http_handlers = {
    ...
    {api::cryptonoted::SyncBlocks::bin_method(), bin_method(&Node::on_wallet_sync3)},
    {api::cryptonoted::SyncMemPool::bin_method(), bin_method(&Node::on_sync_mempool3)},
...
const std::unordered_map<std::string, Node::JSONRPCHandlerFunction> Node::m_jsonrpc_handlers = {
    {api::cryptonoted::GetLastBlockHeaderLegacy::method(), json_rpc::make_member_method(&Node::on_get_last_block_header)},
    {api::cryptonoted::GetBlockHeaderByHashLegacy::method(),
        json_rpc::make_member_method(&Node::on_get_block_header_by_hash)},
    {api::cryptonoted::GetBlockHeaderByHeightLegacy::method(),
        json_rpc::make_member_method(&Node::on_get_block_header_by_height)},
    {api::cryptonoted::GetBlockTemplate::method(), json_rpc::make_member_method(&Node::on_getblocktemplate)},
    {api::cryptonoted::GetBlockTemplate::method_legacy(), json_rpc::make_member_method(&Node::on_getblocktemplate)},
    {api::cryptonoted::GetCurrencyId::method(), json_rpc::make_member_method(&Node::on_get_currency_id)},
    {api::cryptonoted::GetCurrencyId::method_legacy(), json_rpc::make_member_method(&Node::on_get_currency_id)},
    {api::cryptonoted::SubmitBlock::method(), json_rpc::make_member_method(&Node::on_submitblock)},
    {api::cryptonoted::SubmitBlockLegacy::method(), json_rpc::make_member_method(&Node::on_submitblock_legacy)},
    {api::cryptonoted::GetRandomOutputs::method(), json_rpc::make_member_method(&Node::on_get_random_outputs3)},
    {api::cryptonoted::GetStatus::method(), json_rpc::make_member_method(&Node::on_get_status3)},
    {api::cryptonoted::GetStatus::method2(), json_rpc::make_member_method(&Node::on_get_status3)},
    {api::cryptonoted::SendTransaction::method(), json_rpc::make_member_method(&Node::handle_send_transaction3)},
    {api::cryptonoted::CheckSendProof::method(), json_rpc::make_member_method(&Node::handle_check_send_proof3)},
    {api::cryptonoted::SyncBlocks::method(), json_rpc::make_member_method(&Node::on_wallet_sync3)},
    {api::cryptonoted::SyncMemPool::method(), json_rpc::make_member_method(&Node::on_sync_mempool3)}};

bool Node::on_get_random_outputs3(http::Client *, http::RequestData &&, json_rpc::Request &&,
    api::cryptonoted::GetRandomOutputs::Request &&request, api::cryptonoted::GetRandomOutputs::Response &response) {
...
api::cryptonoted::GetStatus::Response Node::create_status_response3() const {
	api::cryptonoted::GetStatus::Response res;
...
bool Node::on_get_status3(http::Client *who, http::RequestData &&raw_request, json_rpc::Request &&raw_js_request,
    api::cryptonoted::GetStatus::Request &&req, api::cryptonoted::GetStatus::Response &res) {
...
bool Node::on_wallet_sync3(http::Client *, http::RequestData &&, json_rpc::Request &&json_req,
    api::cryptonoted::SyncBlocks::Request &&req, api::cryptonoted::SyncBlocks::Response &res) {
	...
	if (req.max_count > api::cryptonoted::SyncBlocks::Request::MAX_COUNT) {
...
bool Node::on_sync_mempool3(http::Client *, http::RequestData &&, json_rpc::Request &&,
    api::cryptonoted::SyncMemPool::Request &&req, api::cryptonoted::SyncMemPool::Response &res) {
...
bool Node::handle_send_transaction3(http::Client *, http::RequestData &&, json_rpc::Request &&,
    api::cryptonoted::SendTransaction::Request &&request, api::cryptonoted::SendTransaction::Response &response) {
	NOTIFY_NEW_TRANSACTIONS::request msg;
	Transaction tx;
...
bool Node::handle_check_send_proof3(http::Client *, http::RequestData &&, json_rpc::Request &&,
    api::cryptonoted::CheckSendProof::Request &&request, api::cryptonoted::CheckSendProof::Response &response) {
	Transaction tx;
	SendProof sp;
	try {
...
*/
>>>>>>> 15c463360ec17ef10da96da016f30bce13467f24

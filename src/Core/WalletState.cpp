...
void WalletPreparatorMulticore::thread_run() {
	while (true) {
		...
		api::cryptonoted::SyncBlocks::SyncBlock sync_block;
...
void WalletPreparatorMulticore::cancel_work() {
	std::unique_lock<std::mutex> lock(mu);
	work = api::cryptonoted::SyncBlocks::Response();
	prepared_blocks.clear();
	work_counter += 1;
}

void WalletPreparatorMulticore::start_work(const api::cryptonoted::SyncBlocks::Response &new_work,
...
bool WalletState::sync_with_blockchain(api::cryptonoted::SyncBlocks::Response &resp) {
...
bool WalletState::sync_with_blockchain(const api::cryptonoted::SyncMemPool::Response &resp) {

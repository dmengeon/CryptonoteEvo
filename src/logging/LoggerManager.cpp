...
void LoggerManager::configure(const JsonValue &val) {
	...
	if (val.contains("globalDisabledCategories")) {
		...
		if (gdc.is_array()) {
			...
			throw std::runtime_error("parameter global_disabled_categories has wrong type");
		}
	}

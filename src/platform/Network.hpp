...
public:
	...
	virtual size_t read_some(void *val, size_t count) override;  // reads 0..count-1, if returns 0 (incoming buffer
	                                                             // empty) would fire rw_handler or d_handler in future
	virtual size_t write_some(const void *val, size_t count)
	    override;  // writes 0..count-1, if returns 0 (outgoing buffer full) will fire rw_handler or d_handler in future
...
public:
	...
	bool accept(TCPSocket &socket,
	    std::string &accepted_addr) {  // if accept returns false, will fire accept_handler in future
		return false;
	}
	bool accept(TCPSocket &socket) {
		std::string a;

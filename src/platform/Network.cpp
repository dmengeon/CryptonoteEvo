...
bool TCPSocket::connect(const std::string &addr, uint16_t port) {
	...
	if (ssl_addr.first) {
		...
		QObject::connect(
		    impl.get(), &QAbstractSocket::bytes_written, [this](qint64 bytes) { this->rw_handler(true, true); });
...
bool TCPSocket::connect(const std::string &addr, uint16_t port) {
	...
	CFRelease(host);
	host = nullptr;
	//	CFReadStreamSetProperty(read_stream, NSStreamSocketSecurityLevelKey, security_dict_ref);
	if (ssl_addr.first) {
	...
	CFReadStreamOpen(read_stream);    // TODO check err
	CFWriteStreamOpen(write_stream);  // TODO check err
	return true;
}

...
void random_keys(PublicKey &pub, SecretKey &sec);

inline KeyPair random_keypair(){
	KeyPair k;
	crypto::random_keys(k.publicKey, k.secretKey);
	return k;
}
...
bool derive_public_key(const KeyDerivation &derivation, size_t output_index,
...
void derive_secret_key(const KeyDerivation &derivation, std::size_t output_index,
...
bool underive_public_key(const KeyDerivation &derivation, size_t output_index,
...
bool check_signature(const Hash &prefix_hash, const PublicKey &pub, const Signature &sig, bool * key_corrupted = nullptr);
...
// returns false if keys are corrupted/invalid
bool generate_ring_signature(const Hash &prefix_hash, const KeyImage &image,
									const PublicKey *const pubs[], std::size_t pubs_count,
									const SecretKey &sec, std::size_t sec_index,
									Signature sigs[]);
bool check_ring_signature(const Hash &prefix_hash, const KeyImage &image,
								 const PublicKey *const pubs[], size_t pubs_count,
								 const Signature sigs[], bool checkKeyImage, bool * key_corrupted = nullptr);
...
inline bool generate_ring_signature(const Hash &prefix_hash, const KeyImage &image,
        ...
        const SecretKey &sec, size_t sec_index, Signature sigs[]) {
...
bool generate_send_proof(const PublicKey &txkey_pub, const SecretKey &txkey_sec,
	const PublicKey &receiver_view_key_pub, const KeyDerivation &derivation, const Hash &message_hash, Signature &proof);

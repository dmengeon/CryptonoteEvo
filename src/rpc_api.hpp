// Copyright (c) 2012-2017, The CryptoNote developers, The Bytecoin developers, [ ] developers
//
...
// bool - Bool
// Amount, SignedAmount, Height, Timestamp, UnlockMoment, Difficulty, (u)int - Number. cryptonote does not use fractional numbers, but uses numbers as large as 2^64-1 for amounts, which is larger than 2^53 exactly representable in double or JavaScript Number
//     amounts large than ~91 million BCN cannot be represented exactly in JavaScript and other platforms using IEEE 64-bit floating numbers, so you should use appropriate json/bigint library to handle large amounts
// std::string, Hash, PublicKey, SecretKey, KeyImage, BinaryArray - String (hex)
// std::vector - Array
// std::map, struct - Object
// cryptonote does not use Null, you should specify empty Array as [], empty string as "", empty Object as {}
namespace cryptonote { namespace api {

struct EmptyStruct {}; // Used as a typedef for empty requests, which we have a lot

typedef int32_t HeightOrDepth; // If >= 0 - interpret as Height, if < 0 interpret as Depth, e.g. -1 means top_block_height, -4 means 3 blocks back from topBlockHeight
...
struct Output {
	...
	uint32_t index_in_transaction = 0; // # of output, output keys depend on transaction_public_key and this index, so they are different for the same address
	// Added from block
	Height height = 0;
...
struct Transfer {
	...
	bool ours = false; // true for addresses in wallet, false for others. Other addresses are recognized only for transactions which have proof data stored in <wallet_file>.history/
	bool locked = false; // locked transfers should not be added to balance immediately. They will be unlocked later and sent via GetTransfers.Response.unlocked_transfers
...
struct Transaction {
	...
	uint32_t anonymity = 0; // recommended to set to DEFAULT_ANONYMITY_LEVEL for new transactions, for existing transactions min(input anonymity) will be returned
	...
	Timestamp timestamp = 0; // Timestamp of block if transaction is in block. For mempool transactions this is the time node first seen this transaction.
};
...
struct BlockHeader {
	...
	uint32_t effective_size_median = 0; // max(100000, size_median) for block version 3, allows sudden peaks in network load.
	Timestamp timestamp_median = 0;
	Timestamp timestamp_unlock = 0; // Can be used for guaranteed output unlocking, as 1. block.timestamp_unlock < block.timestamp and 2. nextBlock.timestamp_unlock >= currentBlock.timestamp_unlock
...
struct Block {
	...
	std::vector<api::Transaction> transactions; // If got from walletd, will contain only transactions with transfers we can view.
};
...
struct Balance {
	...
	Amount total() const { return spendable + spendable_dust + locked_or_unconfirmed; } // This fun serves as a documentation.
};
...
enum return_code {
	...
	WALLET_WITH_THE_SAME_VIEWKEY_IN_USE = 210, // Another walletd instance is using the same wallet file or another wallet file with the same view key.
...
struct GetStatus {
	...
	struct Request {
		Hash top_block_hash;
		uint32_t transaction_pool_version = 0; // Pool version is reset to 1 on every new block. Pool version is incremented on every modification to pool.
		...
		bool operator==(const Request &other) const { return top_block_hash == other.top_block_hash && transaction_pool_version == other.transaction_pool_version && outgoing_peer_count == other.outgoing_peer_count && incoming_peer_count == other.incoming_peer_count; }
	...
	struct Response : public Request { // Response and Request have fields in common.
		...
		uint32_t next_block_effective_median_size = 0; // If your tx size is larger, chance it will not be included in block for a long time (or never).
	};
};
...
struct CreateAddresses {
	...
	struct Request {
		std::vector<SecretKey> secret_spend_keys; // Leave corresponding key hex string empty to generate keypairs. ["", "", ""] will generate three fresh random addresses
		Timestamp creation_timestamp = 0; // If you add addresses created in the past, specify an (oldest) timestamp. If any of those addresses not found in wallet cache file, wallet will resync starting from the creation timestamp and remember that timestamp for every added address. Zero means no timestamp, wallet will use now()
	};
	struct Response {
		std::vector<std::string> addresses;
...
struct GetBalance {
	...
	// Execution time is proportional to top_block_height - height, so keep height reasonably recent (3-30 confirmations works for most usage cases)
	struct Request {
...
struct GetTransfers { // Can be used incrementally by high-performace clients to monitor incoming transfers
	...
	struct Request {
		...
		Height to_height = std::numeric_limits<uint32_t>::max(); // Up to, and including to_height. Will return transfers in mempool if to_height > top_block_height
		bool forward = true; // determines order of blocks returned, additionally if desired_transactions_count set, then this defines if call starts from from_height forward, or from to_height backwards
		uint32_t desired_transactions_count = std::numeric_limits<uint32_t>::max(); // Will return this number of transactions or a bit more, It can return more, because this call always returns full blocks
	};
	struct Response {
		...
		Height next_from_height = 0; // When desired_transactions_count != max you can pass next* to corresponding Request fields to continue iteration
...
struct CreateTransaction {
	...
	struct Request {
		api::Transaction transaction; // You fill only basic info (anonymity, optional unlock_time, optional payment_id) and transfers. All positive transfers (amount > 0) will be added as outputs. For all negative transfers (amount < 0), spendable for requested sum and address will be selected and added as inputs
		std::string spend_address; // If this is not empty, will spend (and optimize) outputs for this address to get neccessary funds. Otherwise will spend any output in the wallet
		bool any_spend_address = false; // if you set spend_address to empty, you should set any_spend_address to true. This is protection against client bug when spend_address is forgotten or accidentally set to null, etc
		std::string change_address; // Change will be returned to change_address.
		HeightOrDepth confirmed_height_or_depth = -DEFAULT_CONFIRMATIONS - 1; // Mix-ins will be selected from the [0..confirmed_height] window. Reorganizations larger than confirmations may change mix-in global indices, making transaction invalid.
		SignedAmount fee_per_byte = 0; // Fee of created transaction will be close to the size of tx * fee_per_byte. You can check it in response.transaction.fee before sending, if you wish
		std::string optimization; // Wallet outputs optimization (fusion). Leave empty to use normal optimization, good for wallets with balanced sends to recieves count. You can save on a few percent of fee (on average) by specifying "minimal" for wallet receiving far less transactions than sending. You should use "aggressive" for wallet recieving far more transactions than sending, this option will use every opportunity to reduce number of outputs. For better optimization use as little anonymity as possible. If anonymity is set to 0, wallet will prioritize optimizing out dust and crazy (large but not round) denominations of outputs.
		bool save_history = true; // If true, wallet will save encrypted transaction data (~100 bytes per used address) in <wallet_file>.history/. With this data it is possible to generate public-checkable proofs of sending funds to specific addresses.
	};
	struct Response {
...
struct SendTransaction {
	...
	struct Response {
		std::string send_result; // "broadcast" if added to pool. "increasefee" if not added to pool because it is full and transaction fee is not enough to replace other transaction.
	};
};
...
struct CreateSendProof {
	...
	struct Request {
		...
		std::string message; // Add any user message to proof. Changing message will invlidate proof (which works like digital signature of message)
...
struct GetTransaction {
	...
	struct Response {
		api::Transaction transaction; // empty transaction no hash returned if this transaction contains no recognizable transfers
	};
};

}}}

// These messages encoded in JSON can be sent via http url /json_rpc3 to cryptonoted rpc address:port
// or to binMethod() url encoded in unspecified binary format
namespace cryptonote { namespace api { namespace cryptonoted {

inline std::string url() { return "/json_rpc"; }

struct GetStatus {
	static std::string method() { return "get_node_status"; } // getNodeStatus works directly or through wallet tunnel
	static std::string method2() { return "get_status"; } // getStatus gets either status of node (if called on node) or wallet (if called on wallet)

	typedef walletd::GetStatus::Request Request;
	typedef walletd::GetStatus::Response Response;
};

struct SyncBlocks { // Used by walletd, block explorer, etc to sync to cryptonoted
...
//		bool send_signatures = false; // Signatures are checked by cryptonoted so usually they are of no interest
	};
	struct SyncBlock {
		api::BlockHeader header;
		cryptonote::Block block; // the only method returning actual Block from blockchain, not api::Block
	...
	struct Response {
		...
		GetStatus::Response status; // We save roundtrip during sync by alst sending status here
	};
};
...
struct SyncMemPool { // Used by walletd sync process
	...
	struct Response {
		...
		std::vector<api::Transaction> added_transactions; // binary version of this method returns only hash and timestamp here
...
struct GetRandomOutputs {
	...
	struct Response {
		std::map<Amount, std::vector<api::Output>> outputs; // can have less outputs than asked for some amounts, if blockchain lacks enough
	};
};

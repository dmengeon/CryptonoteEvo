...
/*bool createTxExtraWithPaymentId(const std::string &paymentIdString, BinaryArray &extra) {
    ...
    cryptonote::set_payment_id_to_transaction_extra_nonce(extraNonce, paymentIdBin);

    if (!cryptonote::add_extra_nonce_to_transaction_extra(extra, extraNonce)) {
        return false;
    }
...
static void doSerialize(cryptonote::TransactionExtraMergeMiningTag &tag, seria::ISeria &s) {
	s.begin_object();
...
void seria::ser(cryptonote::TransactionExtraMergeMiningTag &v, ISeria &s) {
	if (s.is_input()) {
		std::string field;
		s(field);

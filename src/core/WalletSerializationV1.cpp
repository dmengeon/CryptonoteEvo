...
version(version) {}
        WalletTransferDto(const cryptonote::WalletTransfer &tr, uint32_t version)
...
/*void serialize(WalletRecordDto &v, cryptonote::ISerializer &s) {
...
/*void serialize(ObsoleteSpentOutputDto &v, cryptonote::ISerializer &s) {
        s(v.amount, "amount");
...
void serialize(ObsoleteChangeDto &v, cryptonote::ISerializer &s) {
        s(v.tx_hash, "transaction_hash");
        s(v.amount, "amount");
}

void serialize(UnlockTransactionJobDto &v, cryptonote::ISerializer &s) {
        s(v.block_height, "block_height");
        s(v.transaction_hash, "transaction_hash");
        s(v.wallet_index, "wallet_index");
}

void serialize(WalletTransactionDto &v, cryptonote::ISerializer &s) {
        typedef std::underlying_type<cryptonote::WalletTransactionState>::type
StateType;
        ...
        cryptonote::serializeBlockHeight(s, v.block_height, "block_height");
        s(v.hash, "hash");
...
void serialize(WalletTransferDto &v, cryptonote::ISerializer &s) {
        s(v.address, "address");
        s(v.amount, "amount");
...
std::string decrypt(const std::string &cipher, cryptonote::WalletSerializerV1::CryptoContext &crypto_ctx) {
	std::string plain;
...
template<typename Object>
void deserializeEncrypted(Object &obj, const std::string &name, cryptonote::WalletSerializerV1::CryptoContext &crypto_ctx,
...
/*cryptonote::WalletTransaction convert(const cryptonote::WalletLegacyTransaction
&tx) {
...
void WalletSerializerV1::load_wallet_v1(common::IInputStream &source, const crypto::chacha8_key &key) {
	...
	//	cryptonote::BinaryInputStreamSerializer serializer(decrypted_stream);
...
/*void
WalletSerializerV1::loadWalletV1Details(cryptonote::BinaryInputStreamSerializer&
serializer) {
...
void WalletSerializerV1::load_wallets(common::IInputStream &source, CryptoContext &crypto_ctx) {
	...
	for (uint64_t i = 0; i < count; ++i) {
		...
		//		wallet.container =
		// nullptr;//reinterpret_cast<cryptonote::ITransfersContainer*>(i); //dirty
		// hack.
		// container

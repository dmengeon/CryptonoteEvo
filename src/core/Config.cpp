...
static void parse_peer_and_add_to_container(std::string str, std::vector<cryptonote::NetworkAddress> &container) {
...
const static UUID CRYPTONOTE_NETWORK = {{0x11, 0x10, 0x01, 0x11, 0x11, 0x00, 0x01, 0x01, 0x10, 0x11, 0x00, 0x12, 0x10,
...
Config::Config(common::CommandLine &cmd)
	...
	if (const char *pa = cmd.get("--cryptonoted-authorization")) {
		// go to https://www.base64encode.org and encode username:password there
		cryptonoted_authorization = pa;
	}
	if (const char *pa = cmd.get("--cryptonoted-bind-address")) {
		if (!common::parse_ip_address_and_port(cryptonoted_bind_ip, cryptonoted_bind_port, pa))
			throw std::runtime_error("Wrong address format " + std::string(pa) + ", should be ip:port");
	}
	if (const char *pa = cmd.get("--cryptonoted-remote-address")) {
		...
		if (std::string(pa).find(prefix) == 0) {
#if !CRYPTONOTED_SSL
			throw std::runtime_error("Using https in --cryptonoted-remote-address impossible - this binary is built without OpenSSL");
#endif
			std::string sip;
			std::string sport;
			...
			if (!common::parse_ip_address_and_port(cryptonoted_remote_ip, cryptonoted_remote_port, pa))
				throw std::runtime_error("Wrong address format " + std::string(pa) + ", should be ip:port");
		}
	}

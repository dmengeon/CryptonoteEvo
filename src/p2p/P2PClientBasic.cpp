...
cryptonote::P2PClientBasic::LevinHandlerFunction levin_method(void (cryptonote::P2PClientBasic::*handler)(Cmd &&)) {
...
void P2PClientBasic::on_request_ready() {
	...
	while (read_next_request(header, body)) {
		try {
			...
			std::cout << "generic cryptonote::P2P cmd={" << cmd.command << "} " << cmd.is_response << " " << cmd.is_notify
			          << " {"
			          << "}" << std::endl;

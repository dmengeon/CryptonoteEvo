...
crypto::Hash cryptonote::proof_of_trust::get_hash() const {
	std::string s;
...
// P2pProtocolTypes
void ser(cryptonote::UUID &v, seria::ISeria &s) { s.binary(&v, sizeof(v)); }

void ser_members(cryptonote::PeerlistEntry &v, seria::ISeria &s) {
...
void ser_members(cryptonote::NetworkAddress &v, seria::ISeria &s) {
	seria_kv("ip", v.ip, s);
	seria_kv("port", v.port, s);
}

void ser_members(cryptonote::connection_entry &v, seria::ISeria &s) {
	seria_kv("adr", v.adr, s);
	seria_kv("id", v.id, s);
	seria_kv("is_income", v.is_income, s);
}

void ser_members(cryptonote::CoreStatistics &v, seria::ISeria &s) {
...
void ser_members(cryptonote::network_config &v, seria::ISeria &s) {
...
void ser_members(cryptonote::COMMAND_HANDSHAKE::request &v, seria::ISeria &s) {
	seria_kv("node_data", v.node_data, s);
	seria_kv("payload_data", v.payload_data, s);
}

void ser_members(cryptonote::COMMAND_HANDSHAKE::response &v, seria::ISeria &s) {
	seria_kv("node_data", v.node_data, s);
	seria_kv("payload_data", v.payload_data, s);
	serialize_as_binary(v.local_peerlist, "local_peerlist", s);
}

void ser_members(cryptonote::COMMAND_TIMED_SYNC::request &v, seria::ISeria &s) {
	seria_kv("payload_data", v.payload_data, s);
}

void ser_members(cryptonote::COMMAND_TIMED_SYNC::response &v, seria::ISeria &s) {
	seria_kv("local_time", v.local_time, s);
	seria_kv("payload_data", v.payload_data, s);
	serialize_as_binary(v.local_peerlist, "local_peerlist", s);
}

void ser_members(cryptonote::COMMAND_PING::request &v, seria::ISeria &s) {}

void ser_members(cryptonote::COMMAND_PING::response &v, seria::ISeria &s) {
...
void ser_members(cryptonote::COMMAND_REQUEST_STAT_INFO::request &v, seria::ISeria &s) { seria_kv("tr", v.tr, s); }

void ser_members(cryptonote::COMMAND_REQUEST_STAT_INFO::response &v, seria::ISeria &s) {
	seria_kv("version", v.version, s);
...
void ser_members(cryptonote::COMMAND_REQUEST_NETWORK_STATE::request &v, seria::ISeria &s) { seria_kv("tr", v.tr, s); }

void ser_members(cryptonote::COMMAND_REQUEST_NETWORK_STATE::response &v, seria::ISeria &s) {
...
void ser_members(cryptonote::RawBlockLegacy &v, seria::ISeria &s) {
	...
	if (s.isInput()) {
		...
		std::transform(other_transactions.begin(), other_transactions.end(), std::back_inserter(v.transactions),
		    [](const std::string &s) { return cryptonote::BinaryArray(s.data(), s.data() + s.size()); });
	} else {
		...
		std::transform(v.transactions.begin(), v.transactions.end(), std::back_inserter(other_transactions),
		    [](cryptonote::BinaryArray &s) { return std::string(s.begin(), s.end()); });
		seria::seria_kv("block", other_block, s);
		seria::seria_kv("txs", other_transactions, s);
	}
}

void ser_members(cryptonote::NOTIFY_NEW_BLOCK::request &v, seria::ISeria &s) {
	seria_kv("b", v.b, s);
...
void ser_members(cryptonote::NOTIFY_NEW_TRANSACTIONS::request &v, seria::ISeria &s) {
	...
	if (s.isInput()) {
		...
		std::transform(v.txs.begin(), v.txs.end(), std::back_inserter(transactions),
		    [](const cryptonote::BinaryArray &s) { return std::string(s.begin(), s.end()); });
		seria::seria_kv("txs", transactions, s);
	}
}

void ser_members(cryptonote::NOTIFY_REQUEST_GET_OBJECTS::request &v, seria::ISeria &s) {
	serialize_as_binary(v.txs, "txs", s);
	serialize_as_binary(v.blocks, "blocks", s);
}

void ser_members(cryptonote::NOTIFY_RESPONSE_GET_OBJECTS::request &v, seria::ISeria &s) {
...
void ser_members(cryptonote::NOTIFY_REQUEST_CHAIN::request &v, seria::ISeria &s) {
	serialize_as_binary(v.block_ids, "block_ids", s);
}

void ser_members(cryptonote::NOTIFY_RESPONSE_CHAIN_ENTRY::request &v, seria::ISeria &s) {
...
void ser_members(cryptonote::NOTIFY_REQUEST_TX_POOL::request &v, seria::ISeria &s) {
	serialize_as_binary(v.txs, "txs", s);
}
}
